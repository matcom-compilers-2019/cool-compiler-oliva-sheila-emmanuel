...
namespace Logic
{
    ...
    public class Transpiler : coolgrammarBaseVisitor<Node>
    {
        ...
        public override Node VisitIf_else([NotNull] coolgrammarParser.If_elseContext context)
        {
        ...
        public override Node VisitIf_then([NotNull] coolgrammarParser.If_thenContext context)
        {
            Expr exp1 = (Expr)Visit(context.expr(0));
            Expr exp2 = (Expr)Visit(context.expr(1));

            return new If_Else(exp1, exp2, null);
        }

        public override Node VisitNeg_bit([NotNull] coolgrammarParser.Neg_bitContext context)
        {
            Expr exp = (Expr)Visit(context.expr());
            return new UnaryExpr(exp, "~");
        }


        public override Node VisitNot_exp([NotNull] coolgrammarParser.Not_expContext context)
        {
            Expr exp = (Expr)Visit(context.expr());
            return new UnaryExpr(exp, "not");
        }
