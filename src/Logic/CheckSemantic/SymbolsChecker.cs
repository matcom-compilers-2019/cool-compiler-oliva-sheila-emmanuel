...
namespace Logic.CheckSemantic
{
    public class SymCheckerVisitor : IVisitorAST<bool>
    {
        ...
        public bool Visit(Node node)
        {
            ...
            if (node is Branch) return Visit((Branch)node);
        ...
        public bool Visit(Expr node)
        {
            ...
            if (node is CASE_OF) return Visit((CASE_OF)node);
        ...
        public bool Visit(Branch node)
        {
            Context.DefineSymbol(node.formal.name.name, Context.GetType(node.formal.type.s));
